...
nzv(tidy_training,saveMetrics = TRUE)
...
dim(tidy_testing)
```
I did not remove the "user_name" factor variable. The reason is my suspection that different persons carry out the exercises differntly, so the information about the subject might be beneficial for the prediction model. 
However, I am not sure if this can be called "cheating" if we actually want to predict the type of exercise based solely on sensor data, and not using any kind of meta data.

Now we further reduce the datasets by removing all columns which contain missing values in any row. When looking at the raw dataset, it becomes clear that whenever there is missing a value in some row, it is missing in almost all of the other rows as well. Predicting with such "variables" is certainly not a good idea, so we remove those columns entirely.
```{r}
...
## Training the model
After the (crucial) part of getting clean training and testing data, it is now time to decide for a model. Needless to say this is a critical decision. I tried different algorithms, e.g. `rpart`:
```{r}
...
cm_rpart
```
The overall accuracy of this model is `r cm_rpart$overall[1] * 100`%, which is really bad.

I obtained the best results with a random forest model, which is also mentioned frequently in the lectures as one of the best methods in Kaggle contests:
```{r}
...
# Faster version of the above command which is more or less equivalent to the first randomForest command; the do.trace flag results in verbose output
...
cm_rforest
```
This model gives an extremely good out-of-sample accuracy of `r cm_rforest$overall[1] * 100`%.

## Predictions on unknown data
Note that loading and preprocessing of the validation data has already been carried out simultaneously with the training/testing datasets. Predicting is straightforward:
```{r}
